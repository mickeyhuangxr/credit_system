      ...
      <el-table
        ...
        <el-table-column label="操作"  width="400">
          <template slot-scope="scope">
            <el-button size="small" type="primary" @click="check(scope.row)" :disabled="scope.row.status==4">查看</el-button>
            ...
            <el-button size="small" type="info" @click="firstReject(scope.row)" :disabled="scope.row.result=='pass'||scope.row.result=='reject'" >拒绝</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <!--点击编辑的弹层-->
    <el-dialog title="终审-查看" :visible.sync="dialogFormVisible">
      <el-form :model="form">
...
<script>
  import {loanEndQuery,loanEndCheck,loanEndPass ,loanEndReject} from '../../api/artical'
  ...
  export default {
    ...
    methods:{
      getList(){    //获取数据--挂载使用
        loanEndQuery(this.query).then(res=> {
      ...
      check(row){
        ...
        loanEndCheck({id:row.loan_id}).then(res=>{
          ...
          console.log('查看终审的数据失败')
        })
      },
      //点击通过按钮
      firstPass(row){
        console.log(row)
        loanEndPass({appId:row.id,loanId:row.loan_id}).then(res=>{
      ...
      //点击拒绝按钮
      firstReject(row){
        loanEndReject({appId:row.id,loanId:row.loan_id}).then(res=>{
          console.log(res.data)
          // if(res.data.code=='20000'){
          Message({
            message: '终审拒绝',
